import { ShieldCheck, BookOpen, Microscope, Droplet, AlertTriangle, FlaskConical } from "lucide-react";
import { Container, Section, Eyebrow } from "@/components/ui/Container";
import { BodyAtmosphere } from "@/components/water/BodyAtmosphere";
import { ZipCodeHero } from "@/components/water/ZipCodeHero";

export const metadata = {
  title: "Find your water report",
  description:
    "Enter your U.S. ZIP code for an independent water quality report — drawn from EWG's Tap Water Database.",
};

const WHAT_YOU_GET = [
  {
    icon: AlertTriangle,
    title: "Every flagged contaminant",
    detail:
      "Each substance EWG measured above its health-protective guideline — with the multiplier, the source category, and what removes it.",
  },
  {
    icon: FlaskConical,
    title: "EWG vs. EPA gap, made obvious",
    detail:
      "Federal legal limits next to EWG's stricter health guidelines so you can see when your utility is in legal compliance but still posting concerning numbers.",
  },
  {
    icon: Droplet,
    title: "The utility itself, plainly",
    detail:
      "Population served, primary source water, cities covered. No EPA jargon — written so a non-engineer can use it.",
  },
];

export default function ReportLandingPage() {
  return (
    <>
      <section className="relative overflow-hidden bg-midnight text-white py-24 md:py-32 min-h-[80vh] flex items-center">
        <div className="absolute inset-0 pointer-events-none">
          <div
            className="absolute -top-1/4 left-1/4 w-[60vw] h-[60vw] max-w-[800px] max-h-[800px] rounded-full blur-3xl opacity-35"
            style={{
              background:
                "radial-gradient(circle at center, rgba(0,180,216,0.55), transparent 60%)",
            }}
          />
          <div
            className="absolute -bottom-1/4 -right-1/4 w-[50vw] h-[50vw] max-w-[700px] max-h-[700px] rounded-full blur-3xl opacity-25"
            style={{
              background:
                "radial-gradient(circle at center, rgba(201,166,99,0.5), transparent 60%)",
            }}
          />
          <div className="absolute inset-0 bg-grid-faint opacity-[0.06]" />
        </div>
        <Container size="tight" className="relative text-center w-full">
          <div className="inline-flex items-center gap-3 mb-6">
            <span className="h-px w-10 bg-brass-300" />
            <Eyebrow className="text-brass-300">Reports</Eyebrow>
            <span className="h-px w-10 bg-brass-300" />
          </div>
          <h1 className="display text-display-lg text-white mb-6 text-balance leading-[1.02]">
            Find your
            <em className="not-italic italic font-light text-cyan-300"> tap water report.</em>
          </h1>
          <p className="text-lg md:text-xl text-white/75 mb-12 leading-relaxed font-serif italic max-w-xl mx-auto">
            Enter any 5-digit U.S. ZIP code. We look up the public water systems
            serving that ZIP, surface the EWG-flagged contaminants, and score
            the compliance record.
          </p>
          <ZipCodeHero />

          <div className="mt-12 flex flex-wrap items-center justify-center gap-x-7 gap-y-3 text-xs md:text-sm text-white/70">
            <span className="inline-flex items-center gap-1.5">
              <ShieldCheck className="h-4 w-4 text-cyan-300" /> Live EWG data
            </span>
            <span className="text-white/30">·</span>
            <span className="inline-flex items-center gap-1.5">
              <BookOpen className="h-4 w-4 text-cyan-300" /> Citations on every claim
            </span>
            <span className="text-white/30">·</span>
            <span className="inline-flex items-center gap-1.5">
              <Microscope className="h-4 w-4 text-cyan-300" /> No signup
            </span>
          </div>
        </Container>
      </section>

      <Section className="relative py-20 bg-canvas overflow-hidden">
        <BodyAtmosphere variant="mixed" />
        <Container className="relative">
          <div className="max-w-3xl mb-12">
            <div className="flex items-center gap-3 mb-4">
              <span className="h-px w-10 bg-brass-400/70" />
              <Eyebrow>What you&apos;ll get</Eyebrow>
            </div>
            <h2 className="display text-display-md text-ocean-700 text-balance leading-[1.05]">
              Three things, immediately.
            </h2>
          </div>
          <div className="grid md:grid-cols-3 gap-5 md:gap-6">
            {WHAT_YOU_GET.map((w) => {
              const Icon = w.icon;
              return (
                <div
                  key={w.title}
                  className="relative rounded-3xl border border-line bg-white p-7 md:p-8 shadow-soft overflow-hidden"
                >
                  <span className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-cyan-300 via-cyan-400 to-cyan-300" />
                  <div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-cyan-50 text-cyan-600 mb-5">
                    <Icon className="h-5 w-5" />
                  </div>
                  <h3 className="font-serif text-xl text-ocean-700 mb-2 leading-snug">
                    {w.title}
                  </h3>
                  <p className="text-[15px] text-ink/75 leading-relaxed">
                    {w.detail}
                  </p>
                </div>
              );
            })}
          </div>

          <p className="mt-12 text-sm text-muted text-center max-w-2xl mx-auto leading-relaxed">
            Reports are generated live from EWG&apos;s Tap Water Database the
            first time a ZIP is requested, then cached for 30 days. No
            account. No personal data stored unless you subscribe.
          </p>
        </Container>
      </Section>
    </>
  );
}
